...
const draftThree = async (page) => {
  ...
  await page.getByRole('button', { name: '編成', exact: true }).waitFor();
};
...
if ((await desktop.locator('.synergy-coverage > span').count()) !== 6) {
  throw new Error('Team view does not expose all six synergy capabilities');
}
if ((await desktop.locator('.active-grid .synergy-monster-card').count()) !== 3) {
  throw new Error('Team view does not show three active monsters');
}
await assertNoHorizontalOverflow(desktop, 'Desktop team view');
await desktop.screenshot({ path: '/tmp/code-monsters-synergy-team-desktop.png' });

await desktop.getByRole('button', { name: '特殊配合' }).click();
...
await mobile.screenshot({ path: '/tmp/code-monsters-synergy-team-mobile.png' });
await mobile.getByRole('button', { name: '特殊配合' }).click();
...
console.log(
  JSON.stringify({
    ...
    screenshots: [
      '/tmp/code-monsters-synergy-team-desktop.png',
      ...
      '/tmp/code-monsters-synergy-team-mobile.png',
