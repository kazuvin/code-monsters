...
const results = [];
for (const viewport of [{ name: 'desktop', width: 1440, height: 900, deviceScaleFactor: 1 }, { name: 'mobile', width: 390, height: 844, deviceScaleFactor: 3 }]) {
  const page = await browser.newPage({ viewport, deviceScaleFactor: viewport.deviceScaleFactor });
  const errors = [];
  page.on('pageerror', error => errors.push(error.message));
  await page.goto(targetUrl, { waitUntil: 'networkidle' });
  const buildOverflow = await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth);
  ...
  const battlefield = await page.locator('.side-battlefield').first().evaluate(element => {
  ...
  results.push({ viewport: viewport.name, deviceScaleFactor: viewport.deviceScaleFactor, buildOverflow, battleOverflow, battlefieldCount, spriteCount, battlefield, statusCards, actionBubbleCount, logs, logDialogCount, errors });
  await page.close();
}
await browser.close();
