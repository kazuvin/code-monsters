...
assert.equal(
  canRunCondition('enemyOutOfRange', volt, nearestEnemyTargets),
  true,
  '一番近い敵に対する射程外条件を評価できません',
);
assert.equal(
  isConditionCompatibleWithTarget('enemyOutOfRange', 'nearestEnemy'),
  true,
  '敵対象と射程条件が互換ではありません',
);
assert.equal(
...
repairTeam[0].program = [{ targetId: 'nearestAlly', conditionId: 'allyHpBelow50', actionId: 'field-repair' }];
...
assert.ok(repairStep, '応急修復が回復ステップを生成しません');
assert.equal(repairStep.updates[0]?.id, damagedAlly.instanceId, '応急修復が一番近い味方を回復していません');
assert.equal(repairStep.updates[0]?.values.hp, damagedAlly.hp + 22, '応急修復量がデータ定義と一致しません');
...
jumpTeam[0].program = [{ targetId: 'nearestEnemy', conditionId: 'enemyInRange', actionId: 'vault-over' }];
...
throwTeam[0].program = [{ targetId: 'nearestEnemy', conditionId: 'enemyInRange', actionId: 'shoulder-throw' }];
...
pullTeam[0].program = [{ targetId: 'nearestEnemy', conditionId: 'enemyOutOfRange', actionId: 'pull-in' }];
...
farPullTeam[0].program = [
  { targetId: 'nearestEnemy', conditionId: 'enemyOutOfRange', actionId: 'pull-in' },
  { targetId: 'nearestEnemy', conditionId: 'enemyOutOfRange', actionId: 'approach' },
];
...
assert.ok(
  shop.some((item) => item.kind === 'instruction' && item.id === 'emergency-repair'),
  '緊急修復が初期ショップにありません',
);
