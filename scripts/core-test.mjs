...
assert.ok(volt);
const currentEnemyTargets = selectConditionTargets('currentEnemy', volt, enemies, allies);
assert.equal(
  canRunCondition('enemyOutOfRange', volt, currentEnemyTargets),
  true,
  '現在の標的に対する射程外条件を評価できません',
);
assert.equal(
  isConditionCompatibleWithTarget('enemyOutOfRange', 'currentEnemy'),
  true,
  '敵対象と射程条件が互換ではありません',
);
assert.equal(
  isConditionCompatibleWithTarget('selfHpBelow30', 'currentEnemy'),
  false,
  '敵対象に自己HP条件が表示されます',
);
assert.equal(
  isInstructionCompatibleWithTarget(instructionById.get('pull-in'), 'currentEnemy'),
  true,
  '敵単体に引き寄せを設定できません',
);
assert.equal(
...
jumpTeam[0].program = [{ targetId: 'currentEnemy', conditionId: 'enemyInRange', actionId: 'vault-over' }];
...
throwTeam[0].program = [{ targetId: 'currentEnemy', conditionId: 'enemyInRange', actionId: 'shoulder-throw' }];
...
pullTeam[0].program = [{ targetId: 'currentEnemy', conditionId: 'enemyOutOfRange', actionId: 'pull-in' }];
...
farPullTeam[0].program = [
  { targetId: 'currentEnemy', conditionId: 'enemyOutOfRange', actionId: 'pull-in' },
  { targetId: 'currentEnemy', conditionId: 'enemyOutOfRange', actionId: 'approach' },
];
