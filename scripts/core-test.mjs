...
assert.equal(fighters.filter(fighter=>fighter.team==='ally').length,team.length,'味方編成を戦闘状態へ変換できません');
assert.equal(fighters.filter(fighter=>fighter.team==='enemy').length,ROSTER_CONFIG.enemyUnitIds.length,'敵編成がデータ定義と一致しません');
assert.ok(fighters.every(fighter=>fighter.guardDamageScale===1&&fighter.guardKnockbackScale===1),'初期ガード倍率は1である必要があります');
...
assert.equal(actionCooldown(volt.speed),Math.max(BATTLE_CONFIG.minimumActionCooldownSeconds,BATTLE_CONFIG.baseActionCooldownSeconds/volt.speed));

const ready=fighters.map(fighter=>({...fighter,cooldown:fighter.team==='ally'&&fighter.id==='volt'?0:10}));
const plan=planBattleFrame({fighters:ready,team,dt:BATTLE_CONFIG.tickSeconds,elapsed:BATTLE_CONFIG.tickSeconds,previousElapsed:0});
...
assert.ok(shop.some(item=>item.kind==='unit'&&item.id==='wrath'),'初期ショップ定義が反映されていません');
...
assert.equal(balance.errors,0,`バランス検査にエラーがあります: ${balance.issues.map(issue=>issue.message).join(' / ')}`);
assert.ok(balance.metrics.every(metric=>Number.isFinite(metric.power)&&metric.power>0),'全ユニットの戦力値を算出できません');
...
assert.ok(invalidReport.issues.some(issue=>issue.code==='UNKNOWN_INSTRUCTION'),'壊れたスキル参照を静的検査で検出できません');

console.log(JSON.stringify({fighters:fighters.length,steps:plan.steps.length,balanceWarnings:balance.warnings},null,2));
