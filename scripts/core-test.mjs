...
assert.equal(BATTLE_CONFIG.teamSize, 2, '標準チームサイズが2体ではありません');
assert.equal(team.length, BATTLE_CONFIG.teamSize, 'デフォルト味方編成が2体ではありません');
assert.ok(
  ENCOUNTERS.every((encounter) => encounter.enemyUnitIds.length === BATTLE_CONFIG.teamSize),
  '全ラウンドの標準敵編成が2体ではありません',
);
...
assert.equal(
  isConditionCompatibleWithTarget('targetInRange', 'partner'),
  true,
  '相棒対象と射程条件が互換ではありません',
);
assert.equal(isConditionCompatibleWithTarget('always', 'partner'), false, '相棒対象に「いつでも」が表示されます');
assert.equal(
...
assert.equal(selectConditionTargets('partner', volt, enemies, allies)[0]?.id, 'mender', '2vs2の相棒を選べません');
assert.equal(selectConditionTargets('partner', volt, enemies, allies).length, 1, '相棒対象が複数になっています');
assert.equal(
  isInstructionCompatibleWithTarget(instructionById.get('field-repair'), 'partner'),
  true,
  '相棒にヒールを設定できません',
);
assert.equal(
...
repairTeam[0].program = [{ targetId: 'partner', conditionId: 'partnerHpBelow50', actionId: 'field-repair' }];
...
assert.equal(repairStep.updates[0]?.id, damagedAlly.instanceId, 'ヒールが相棒を回復していません');
...
farRepairTeam[0].program = [{ targetId: 'partner', conditionId: 'partnerHpBelow50', actionId: 'field-repair' }];
...
assert.ok(!farRepairPlan.steps.some((step) => step.flash.kind === 'heal'), '射程範囲外の相棒をヒールしています');
assert.ok(
  farRepairPlan.decisions.some(
    (decision) => decision.actionId === 'field-repair' && decision.outcome === 'skipped' && decision.reason === 'range',
  ),
  '射程範囲外の相棒へのヒールを射程理由で見送っていません',
);
...
supportTeam[0].program = [{ targetId: 'partner', conditionId: 'partnerHpBelow50', actionId: 'field-repair' }];
const supportFighters = createBattleFighters(supportTeam).map((fighter) => ({
  ...
  hp: fighter.instanceId === 'support-volt' ? fighter.maxHp - 60 : fighter.hp,
...
assert.equal(supportHealStep?.updates[0]?.values.hp, 90, 'サポート専用の34HP回復量が適用されていません');
...
assert.deepEqual(
  ...
  { unitId: 'mender', trigger: 'partnerAttackHit', actionId: 'coolant-shot', fixedReaction: true },
...
inspiredSupportTeam[0].program = [{ targetId: 'partner', conditionId: 'targetInRange', actionId: 'tactical-support' }];
...
const inspiredSupportUpdate = inspiredSupportPlan.steps
  .find((step) => step.flash.actionLabel === '戦術支援')
...
for (const id of ['wrath', 'mender', 'toxin', 'shoulder-throw', 'taunt', 'pull-in', 'saturation-fire'])
  assert.ok(sampledShopIds.has(id), `${id} がランダムショップ候補に入りません`);
assert.ok(!sampledShopIds.has('field-repair'), '初期所持の相棒回復がショップ候補に残っています');
...
const debugHealingInput = {
  ...
  conditionId: 'partnerHpBelow50',
  targetSelectorId: 'partner',
  targetMaxHp: 200,
  actorHpRatio: 0.3,
  targetDefense: 5,
  targetWeight: 5,
