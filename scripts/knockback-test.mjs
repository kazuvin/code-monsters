...
const knockbackDistance =
  knockAwayTargetX === null || knockbackTargetX === null ? 0 : Math.abs(knockbackTargetX - knockAwayTargetX);
console.log(
  JSON.stringify(
    {
      normalRowClasses,
      reactionRowClasses,
      legacyReactionEditorCount,
      reactionCopy,
      startingConditions,
      startingActions,
      startingShopInstructions,
      knockAwayCardText,
      knockAwayChoiceText,
      program,
      events,
...
for (const classes of [normalRowClasses, reactionRowClasses]) {
  if (!classes?.includes('code-block') || !classes.includes('sentence-block'))
    throw new Error('通常作戦とリアクションが同じ行UIを使用していません');
}
if (legacyReactionEditorCount !== 0) throw new Error('旧リアクション専用カードUIが残っています');
if (!reactionCopy.includes('REACTION LOOP') || !reactionCopy.includes('もし') || !reactionCopy.includes('なら'))
...
if (
  ...
  !startingActions.some((text) => text.includes('前進する'))
)
  throw new Error('初期行動は「通常攻撃」「前進する」の2つである必要があります');
if (startingShopInstructions.some((text) => text.includes('通常攻撃') || text.includes('前進する')))
  throw new Error('初期行動がショップにも並んでいます');
if (
  !knockAwayCardText.includes('RARE') ||
  !knockAwayCardText.includes('KB出力 120') ||
  !knockAwayCardText.includes('どっか遠くへ、ぽん。じゃあね。')
)
  throw new Error('ショップで技名・遊びのある説明・定量効果を確認できません');
if (
  !knockAwayChoiceText.includes('ちょっと吹き飛ばす') ||
  !knockAwayChoiceText.includes('ABILITY') ||
  !knockAwayChoiceText.includes('基礎DMG 35') ||
  !knockAwayChoiceText.includes('KB出力 120')
)
  throw new Error('行動選択カードで技名・能力値を確認できません');
if (!program.startsWith('1 もし 射程範囲内 なら ちょっと吹き飛ばす'))
...
if (!events.some((event) => event.label === 'KNOCKBACK'))
  throw new Error('「ちょっと吹き飛ばす」の後にノックバックイベントが発生しませんでした');
if (knockbackDistance < 25)
  throw new Error(`「ちょっと吹き飛ばす」のノックバック距離が不足しています: ${knockbackDistance}`);
