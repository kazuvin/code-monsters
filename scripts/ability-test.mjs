...
const reactionSnapshot = async () => {
  ...
  if ((await block.count()) === 0)
    return { exists: false, text: (await editor.innerText()).replace(/\s+/g, ' ').trim() };
  return {
    exists: true,
...
const observeBattle = async (required, maxTicks = 400, extraCheck = () => true) => {
  for (let i = 0; i < maxTicks; i += 1) {
    ...
    for (const className of await page
      ...
      for (const token of className.split(/\s+/).filter((token) => token.startsWith('is-')))
        animationClasses.add(token);
    }
    const followState = await page.locator('.sprite.is-follow').evaluateAll((elements) =>
    ...
    const relayStatuses = await page
      .locator('.sprite')
      .filter({ hasText: 'リレイ' })
      .locator('.status-chip')
      .allTextContents();
...
for (const [reaction, trigger, action] of fixedChecks) {
  if (
    !reaction.exists ||
    !reaction.text.includes(trigger) ||
    !reaction.text.includes(action) ||
    !reaction.triggerDisabled ||
    !reaction.actionDisabled ||
    !reaction.deleteDisabled
  ) {
    throw new Error(`${action}の固定リアクションUIが不正です`);
  }
}
if (voltNormal.includes('追撃') || arrowNormal.includes('緊急離脱'))
  throw new Error('リアクションが通常作戦にも残っています');
if (relayEmpty.exists || !relayEmpty.text.includes('0 / 1')) throw new Error('リレイのリアクション初期状態が不正です');
if (
  !relayNormal.text.includes('高速接近') ||
  relayNormal.conditionDisabled ||
  !relayNormal.actionDisabled ||
  !relayNormal.deleteDisabled
)
  throw new Error('高速接近の通常作戦UIが不正です');
if (
  !relayConfigured.exists ||
...
if (!followDistances.some((distance) => distance > 10))
  throw new Error('射程外のリアクション追撃を確認できませんでした');
...
if (stacking.log <= Math.max(...stacking.sprites))
  throw new Error('ログダイアログがユニットより手前に表示されていません');
