...
profile、gambit、特殊配合のtabを持つ。能力の最終値と成長・個体値・装備内訳、別れcoinの内訳を表示する。奇獣も特殊配合tabを表示し、「この種を作る」「この種を使う」の両方向が存在しないことを明示する。
...
基本45種と奇獣5種、全50種の発見状態、系統filter、詳細、特殊配合を表示する。発見状態はランをまたいで保持する。
