...
モンスター、スキル、イベントのtabを持つ。図鑑一覧は端末の動的viewport全体を使い、headerとfilterの下に残る領域を一覧の内部scrollへ割り当てる。モンスターtabは基本27種と追加7種、全34種の発見状態と系統filterを一覧表示し、カード選択時だけ詳細dialogで成長scanと特殊配合を表示する。スキルtabとイベントtabも一覧から詳細dialogを開く。発見状態はランをまたいで保持する。
...
- 配合候補が複数成立
- モンスター図鑑34種、スキル図鑑、イベント図鑑と各詳細dialogを表示
