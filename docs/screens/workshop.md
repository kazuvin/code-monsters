...
- 装備のレアリティとレアリティ別排出率
- 商品価格と購入可否
- 更新cost、無料更新数、棚固定状態、希少入荷率
- 手持ちまたは同じ棚の商品と白星上昇の特殊配合が成立する商品の予兆
- 希少入荷率、卵の孵化までの保持回数
- 早熟・晩成と役割を示す性能tag
- 配合可能level
- 主力3体が揃っているか
...
profile、gambit、特殊配合のtabを持つ。能力の最終値と成長・個体値・装備内訳を表示する。必要経験値と能力成長量は、レベル2〜10を共通軸にした2段のgrowth scanで表示し、選んだレベルの値と現在levelを読めるようにする。すべてのモンスターで特殊配合tabを表示し、「この種を作る」「この種を使う」の両方向が存在しない場合は明示する。
...
モンスター、スキル、イベントのtabを持つ。図鑑一覧は端末の動的viewport全体を使い、headerとfilterの下に残る領域を一覧の内部scrollへ割り当てる。モンスターtabは基本45種と追加7種、全52種の発見状態と系統filterを一覧表示し、カード選択時だけ詳細dialogで成長scanと特殊配合を表示する。スキルtabとイベントtabも一覧から詳細dialogを開く。発見状態はランをまたいで保持する。
