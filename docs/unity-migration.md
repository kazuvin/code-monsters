...
- purchasable instructions, fixed/default reactions, and ordered programs
...
- ordered one-on-one encounters, including each rival's program, reaction, stat scale, and reward
...
Stable IDs such as `nearestEnemy`, `self`, and `targetNear10` are saved and evaluated. Japanese copy is display data only. This avoids coupling Unity rules to localization.
...
| `src/core/roster.ts` | `RosterFactory.cs` | inventory units and one-on-one battle-state construction |
| `src/core/shop.ts` | `ShopGenerator.cs` | seeded direct-instruction shop generation |
...
## Schema version 22 migration

Remove the equipment registry, starting/enemy equipment IDs, and mixed shop slots. Inventory units now keep their immutable body stats, while the four shop positions sell instructions directly. Remove `air-dash`; `vector-thrust` is usable at any Y coordinate and uses authored braking to cover 18m without residual sliding.

Projectile deliveries add `minimumTravelDistance`. Collision starts only after the projectile crosses that distance, while proximity reactions use `battle.projectileThreatRadius` and may trigger once per fighter and projectile. Add the `enemyProjectileNear` reaction trigger for evasive movement programs.
