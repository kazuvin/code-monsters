...
type MutableFighterFields=Pick<Fighter,'hp'|'x'|'cooldown'|'reactionCooldown'|'guarded'|'guardDamageScale'|'guardKnockbackScale'|'berserk'|'poison'|'attack'|'speed'>;
...
export function isBattleComplete(fighters:Fighter[]):boolean{
  return !fighters.some(fighter=>fighter.team==='ally'&&fighter.hp>0)
    || !fighters.some(fighter=>fighter.team==='enemy'&&fighter.hp>0);
}

export function planBattleFrame({fighters,team,dt,elapsed,previousElapsed}:{
  fighters:Fighter[];
  ...
  const queueReaction=(reactorId:string,trigger:ReactionTrigger,sourceId:string,targetId:string)=>{
    ...
    const target=trigger==='selfHit'
      ? source
      : trigger==='selfHpLow'
        ? nearestEnemy(reactor,next.filter(fighter=>fighter.team!==reactor.team&&fighter.hp>0))
        : eventTarget;
    ...
    if(instruction.action==='berserk'){
      ...
      queueStep({
        ...
        log:{actor:reactor.name,text:`REACTION｜バーサーカーモード｜ATK ${reactor.attack}→${boost.attack} / SPD ${reactor.speed.toFixed(2)}→${boost.speed.toFixed(2)}`,type:'reaction'},
        updates:[{id:reactor.instanceId,values}],
      });
      return;
    }
    ...
    if(!isFollow&&distanceTo(reactor,target)>reactor.range){
      ...
      setNext(reactor.instanceId,values);
      queueStep({
        flash:{id:reactor.instanceId,kind:'miss',attackType:reactor.attackType,actionLabel:`${actionLabel}｜MISS`,reaction:true,n:0},
        log:{actor:reactor.name,text:`REACTION｜${instruction.short} → ${target.name}｜空振り（射程外）`,type:'miss'},
        updates:[{id:reactor.instanceId,values}],
      });
      return;
    }
    ...
    const attackKind=instruction.action==='heavy'||instruction.action==='poison'||instruction.action==='burn'||instruction.action==='follow'?instruction.action:'attack';
    queueStep({
      flash:{id:reactor.instanceId,kind:attackKind,targetId:target.instanceId,attackType:reactor.attackType,actionLabel,reaction:true,n:0},
      log:{actor:reactor.name,text:`REACTION｜${instruction.short} → ${target.name}｜${impact.damage} dmg`,type:'reaction'},
      updates:[
        {id:reactor.instanceId,values:{reactionCooldown:BATTLE_CONFIG.reactionCooldownSeconds}},
        {id:target.instanceId,values:{hp,poison}},
      ],
    });
    if(hp>0&&x!==target.x)queueStep({
      flash:{id:target.instanceId,kind:'hit',actionLabel:'KNOCKBACK',n:0},
      updates:[{id:target.instanceId,values:{x}}],
    });
    if(hp<=0&&target.hp>0)queueStep({flash:{id:target.instanceId,kind:'death',actionLabel:'DOWN',n:0},updates:[]});
  };
  ...
  const triggerHitReactions=(attackerId:string,targetId:string)=>{
    ...
    if(target.hp>0){
      ...
      if(target.hp/target.maxHp<=BATTLE_CONFIG.lowHpThreshold)queueReaction(target.instanceId,'selfHpLow',attacker.instanceId,target.instanceId);
    }
    queueReaction(attacker.instanceId,'selfAttackHit',attacker.instanceId,target.instanceId);
    for(const ally of next.filter(fighter=>fighter.team===attacker.team&&fighter.instanceId!==attacker.instanceId&&fighter.hp>0)){
  ...
  if(elapsed>=BATTLE_CONFIG.overheatStartSeconds){
    ...
    displayNext=displayNext.map(fighter=>{
      ...
      if(hp<=0&&fighter.hp>0)queueStep({flash:{id:fighter.instanceId,kind:'death',actionLabel:'DOWN',n:0},updates:[]});
      return {...fighter,hp};
    });
    next=displayNext.map(fighter=>({...fighter}));
    if(Math.floor(elapsed)!==Math.floor(previousElapsed))logs.push({actor:'SYSTEM',text:`OVERHEAT｜最大HPの ${(rate*100).toFixed(0)}% ダメージ`,type:'hit'});
  }

  for(const fighter of next.filter(candidate=>candidate.hp>0&&candidate.hp/candidate.maxHp<=BATTLE_CONFIG.lowHpThreshold)){
    queueReaction(fighter.instanceId,'selfHpLow',fighter.instanceId,fighter.instanceId);
  }

  for(const ready of [...next].filter(fighter=>fighter.hp>0&&fighter.cooldown<=0).sort((a,b)=>b.speed-a.speed)){
    ...
    const enemyProgram=(DEFAULT_PROGRAMS[actor.id]??[]).map(actionId=>({actionId,conditionId:instructionById.get(actionId)?.condition??'always'}));
    const program=actor.team==='ally'?(team.find(unit=>unit.inventoryId===actor.instanceId)?.program??[]):enemyProgram;
    ...
    for(const block of program.slice(0,actor.programLimit)){
      ...
      if(instruction.action==='move'){
        if(distanceTo(current,nearest)<=current.range){
          queueStep({flash:{id:current.instanceId,kind:'wait',actionLabel:'待機',n:0},log:{actor:current.name,text:`${nearest.name}と対峙｜前線を維持`,type:'info'},updates:[]});
        }else{
          const x=advanceToward(current,nearest,instruction.params.moveDistance??0);
          setNext(current.instanceId,{x});
          queueStep({
            flash:{id:current.instanceId,kind:instruction.visualKind??'move',actionLabel:instruction.short,n:0},
            log:{actor:current.name,text:`${nearest.name}へ${instruction.short}｜戦線 ${Math.round(x)}`,type:'info'},
            updates:[{id:current.instanceId,values:{x}}],
          });
        }
      }else if(instruction.action==='heal'){
        const amount=Math.round(current.role==='SUPPORT'?(instruction.params.supportHealAmount??instruction.params.healAmount??0):(instruction.params.healAmount??0));
        const hp=Math.min(target.maxHp,target.hp+amount);
        setNext(target.instanceId,{hp});
        queueStep({flash:{id:target.instanceId,kind:'heal',actionLabel:'回復',n:0},log:{actor:current.name,text:`${target.name}を ${amount} 修復`,type:'heal'},updates:[{id:target.instanceId,values:{hp}}]});
      }else if(instruction.action==='retreat'){
        const x=retreatFrom(current,instruction.params.moveDistance??0);
        setNext(current.instanceId,{x});
        queueStep({
          flash:{id:current.instanceId,kind:instruction.visualKind??'move',actionLabel:instruction.short,n:0},
          log:{actor:current.name,text:`${nearest.name}から${instruction.short}｜戦線 ${Math.round(x)}`,type:'info'},
          updates:[{id:current.instanceId,values:{x}}],
        });
      }else if(instruction.action==='guard'){
        const values={guarded:true,guardDamageScale:instruction.params.incomingDamageScale??1,guardKnockbackScale:instruction.params.incomingKnockbackScale??1};
        setNext(current.instanceId,values);
        queueStep({flash:{id:current.instanceId,kind:'guard',actionLabel:instruction.short,n:0},log:{actor:current.name,text:'防御姿勢へ移行',type:'info'},updates:[{id:current.instanceId,values}]});
      }else if(instruction.action==='berserk'){
        if(current.berserk){
          queueStep({flash:{id:current.instanceId,kind:'wait',actionLabel:'暴走継続',n:0},log:{actor:current.name,text:'バーサーカーモードはすでに稼働中',type:'info'},updates:[]});
        }else{
          ...
          queueStep({
            ...
            log:{actor:current.name,text:`バーサーカーモード｜ATK ${current.attack}→${boost.attack} / SPD ${current.speed.toFixed(2)}→${boost.speed.toFixed(2)}`,type:'info'},
        ...
        queueStep({flash:{id:current.instanceId,kind:'heal',actionLabel:'強化',n:0},log:{actor:current.name,text:`攻撃出力を +${instruction.params.attackFlat??0} 強化`,type:'heal'},updates:[{id:current.instanceId,values:{attack}}]});
        ...
        queueStep({flash:{id:current.instanceId,kind:'wait',actionLabel:'待機',n:0},log:{actor:current.name,text:'同期タイミングを待機',type:'info'},updates:[{id:current.instanceId,values:{cooldown:waitCooldown}}]});
      }else if(distanceTo(current,target)>current.range&&instruction.action!=='follow'){
        queueStep({
          flash:{id:current.instanceId,kind:'miss',attackType:current.attackType,actionLabel:`${instruction.short}｜MISS`,n:0},
        ...
        const attackKind=instruction.action==='heavy'||instruction.action==='poison'||instruction.action==='burn'||instruction.action==='follow'?instruction.action:'attack';
        queueStep({
          flash:{id:current.instanceId,kind:attackKind,targetId:target.instanceId,attackType:current.attackType,actionLabel:instruction.showAttackTypeLabel?attackTypeLabels[current.attackType]:instruction.short,n:0},
          log:{actor:current.name,text:`${instruction.short} / ${current.attackType} → ${target.name}｜${impact.damage} dmg`,type:'hit'},
          updates:[{id:target.instanceId,values:{hp,poison}}],
        });
        if(hp>0&&x!==target.x)queueStep({flash:{id:target.instanceId,kind:'hit',actionLabel:'KNOCKBACK',n:0},updates:[{id:target.instanceId,values:{x}}]});
        if(hp<=0&&target.hp>0)queueStep({flash:{id:target.instanceId,kind:'death',actionLabel:'DOWN',n:0},updates:[]});
