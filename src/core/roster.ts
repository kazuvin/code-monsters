...
export function createBattleFighters(team:UnitInventoryItem[]):Fighter[]{
  const enemies=ROSTER_CONFIG.enemyUnitIds.map((id,index)=>createInventoryUnit(id,`enemy-template-${id}-${index}`));
  const baseState={z:0,reactionCooldown:0,guarded:false,guardDamageScale:1,guardKnockbackScale:1,berserk:false,poison:0};
  return [
    ...team.map((unit,index)=>({
      ...unit,
      ...baseState,
