...
export function resolveImpact(input: ImpactInput): ImpactResult {
  ...
  const baseDamage = Math.round(
    (input.rawDamage - input.targetDefense * BATTLE_CONFIG.defenseDamageFactor) * guardedScale,
  );
  ...
  const knockbackDistance =
    Math.max(
      BATTLE_CONFIG.minimumKnockbackDistance,
      knockbackPower - input.targetWeight * BATTLE_CONFIG.weightKnockbackFactor,
    ) *
    tankScale *
    guardScale;
