...
describe('light vein build', () => {
  it('defines two payoff paths with complete playable coverage', () => {
    const build = GAME_DATA.buildDesign.builds.find((candidate) => candidate.id === 'light-vein');
    const designs = GAME_DATA.buildDesign.skills.filter((skill) =>
      skill.buildLinks.some((link) => link.buildId === 'light-vein'),
    );

    expect(build?.title).toBe('光脈');
    expect(build?.payoffs.map((payoff) => payoff.title)).toEqual(['枝光', '収光']);
    expect(designs.filter((skill) => skill.status === 'playable')).toHaveLength(10);
    expect(
      new Set(designs.flatMap((skill) => skill.axisLinks.find((link) => link.axisId === 'weapon')?.valueIds ?? [])),
    ).toEqual(new Set(['blade', 'bow', 'device', 'magic', 'cannon']));
  });
  ...
  it('caps the fused three-branch payoff below one enemy health bar', () => {
    ...
    expect(fused).toMatchObject({ value: 2580, stars: 1 });
  ...
  it('caps the fused three-route convergence after the existing merge multiplier', () => {
    ...
    expect(fused).toMatchObject({ value: 2940, mergeMultiplier: 2, stars: 1 });
    expect(fused && 'value' in fused ? fused.value : 0).toBeLessThan(GAME_DATA.units[1].maxHp);
  });
});
