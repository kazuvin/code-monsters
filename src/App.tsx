    ...
    const timer=setInterval(()=>{
      ...
      setFighters(prev=>{
        ...
        const queueReaction=(reactorId:string,trigger:ReactionTrigger,sourceId:string,targetId:string)=>{
          ...
          if(!isFollow&&distanceTo(reactor,target)>reactor.range)return;
          ...
          for(const block of ids.slice(0,actor.programLimit)){
            ...
            if(ins.action==='move'){
              ...
              if(!isFollow&&distanceTo(current,target)>current.range){
                queueStep({
                  flash:{id:current.instanceId,kind:'wait',actionLabel:'射程外',n:Date.now()},
                  log:{actor:current.name,text:`${target.name}が射程外｜${ins.short}を見送り`,type:'skip'},
                  apply:fighters=>fighters,
                });
              }else{
                const ti=next.findIndex(f=>f.instanceId===target.instanceId);
                ...
                const attackKind=ins.action==='heavy'||ins.action==='poison'||ins.action==='burn'||ins.action==='follow'?ins.action:'attack';
                queueStep({
                  flash:{id:current.instanceId,kind:attackKind,targetId:target.instanceId,attackType:current.attackType,actionLabel:isFollow?ins.short:attackTypeLabels[current.attackType],n:Date.now()},
  ...
  return <main className="app-shell">
    ...
    {phase==='build'?<div className="build-layout">
      <section className="workbench">
        ...
        <section className={`reaction-editor ${currentReaction?'armed':'empty'}`}>
          <div className="reaction-head"><div><span className="reaction-beacon"><Zap size={15}/></span><div><small>REACTION / ALWAYS LISTENING</small><h3>リアクション</h3></div></div><b>{currentReaction?'1 / 1':'0 / 1'}</b></div>
          <p>戦闘イベントを常時監視。通常作戦とは別のタイミングで割り込みます。</p>
          {currentReaction?<article className={`reaction-block ${currentReaction.fixedReaction?'fixed-action':''}`}><span className="reaction-state">ARMED</span><div className="reaction-sentence"><button className={editingSlot?.scope==='reaction'&&editingSlot.field==='condition'?'word-slot active':'word-slot'} disabled={currentReaction.fixedReaction} onClick={()=>setEditingSlot({scope:'reaction',index:0,field:'condition'})}>{reactionTriggerLabels.get(currentReaction.trigger)}</button><Zap size={14}/><button className={editingSlot?.scope==='reaction'&&editingSlot.field==='action'?'word-slot active':'word-slot'} disabled={currentReaction.fixedReaction} onClick={()=>setEditingSlot({scope:'reaction',index:0,field:'action'})}>{actionLabel(currentReaction.actionId)}</button></div><button className="remove-reaction" aria-label="リアクションを削除" disabled={currentReaction.fixedReaction} onClick={removeReaction}><X size={14}/></button></article>:<button className="add-reaction" onClick={addReaction}><Zap size={15}/>リアクションを設定</button>}
          <div className="reaction-foot"><span>独立クールダウン</span><b>{REACTION_COOLDOWN.toFixed(1)} SEC</b></div>
        </section>
