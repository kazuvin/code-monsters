...
import type { BattleFlash, ConditionId, Fighter, Instruction, LogItem, ProgramBlock, Rarity, ReactionBlock, ReactionTrigger, UnitDefinition, UnitInventoryItem } from './types';
...
const attackTypeLabels:Record<UnitDefinition['attackType'],string>={melee:'近距離',blunt:'打撃',sniper:'狙撃'};
...
const ShopUnitTrait=({unit}:{unit:UnitDefinition})=>{
  ...
  const metrics=instructionMetrics(instruction,unit);
  return <div className={`shop-unit-trait ${instruction.action==='berserk'?'berserk':''}`}>
    <small>固有リアクション</small>
    <b>{detail.title} → {instruction.short}</b>
...
export function App(){
  ...
  const winner=phase==='result'?(fighters.some(fighter=>fighter.team==='enemy'&&fighter.hp>0)?'敗北':'勝利'):'';

  const addLog=useCallback((actor:string,text:string,type:LogItem['type']='info')=>{
    const t=Math.max(0,elapsedRef.current);
    setLogs(current=>[{id:++logId.current,time:`${Math.floor(t/60)}:${String(Math.floor(t%60)).padStart(2,'0')}`,actor,text,type},...current].slice(0,BATTLE_CONFIG.maxLogEntries));
  },[]);

  const updateSelectedProgram=(updater:(program:ProgramBlock[])=>ProgramBlock[])=>setTeam(units=>units.map((unit,index)=>index===selected?{...unit,program:updater(unit.program)}:unit));
  const updateSelectedReaction=(updater:(reaction:ReactionBlock|null)=>ReactionBlock|null)=>setTeam(units=>units.map((unit,index)=>index===selected?{...unit,reaction:updater(unit.reaction)}:unit));

  const refresh=()=>{
    if(coins<ECONOMY_CONFIG.refreshCost){setToast('コインが足りません');return;}
  ...
  const toggleLock=(key:string)=>setShop(current=>current.map(item=>item.key===key?{...item,locked:!item.locked}:item));

  const buy=(item:ShopItem)=>{
    if(item.kind==='unit'){
      const unit=unitById.get(item.id)!;
      if(coins<unit.price){setToast('コインが足りません');return;}
      ...
      if(coins<instruction.price){setToast('コインが足りません');return;}
      ...
      if(!instruction.reactionOnly)setOwnedConditions(current=>current.includes(instruction.condition)?current:[...current,instruction.condition]);
      setToast(`${instruction.title}を取得しました`);
    }
    setShop(current=>current.filter(candidate=>candidate.key!==item.key));
  };

  const moveInstruction=(index:number,direction:-1|1)=>updateSelectedProgram(program=>{
    const next=[...program];
  ...
  const removeInstruction=(index:number)=>{if(!currentProgram[index]?.fixedAction)updateSelectedProgram(program=>program.filter((_,candidate)=>candidate!==index));};
  ...
  const replaceFocusedSlot=(id:string)=>{
    ...
    if(editingSlot.scope==='reaction'){
      ...
      updateSelectedProgram(program=>program.map((block,index)=>index===editingSlot.index?{...block,conditionId:id as ConditionId}:block));
    }else if(!currentProgram[editingSlot.index]?.fixedAction){
      updateSelectedProgram(program=>program.map((block,index)=>index===editingSlot.index?{...block,actionId:id}:block));
    }
  };
  ...
  useEffect(()=>{
    ...
    const timer=setInterval(()=>{
      ...
      if(queuedStep){setFighters(current=>tickCooldowns(current,dt));return;}
      setFighters(current=>{
  ...
  const teamHp=useMemo(()=>fighters.filter(fighter=>fighter.team==='ally').reduce((total,fighter)=>total+Math.max(0,fighter.hp),0),[fighters]);
  const enemyHp=useMemo(()=>fighters.filter(fighter=>fighter.team==='enemy').reduce((total,fighter)=>total+Math.max(0,fighter.hp),0),[fighters]);
  ...
  const cooldownProgress=(fighter:Fighter)=>Math.max(0,Math.min(1,1-fighter.cooldown/actionCooldown(fighter.speed)));
  ...
  return <main className="app-shell">
    ...
    {phase==='build'?<div className="build-layout">
      <section className="workbench">
        ...
        <div className="unit-tabs">{team.map((unit,index)=><button key={unit.inventoryId} className={selected===index?'active':''} onClick={()=>{setSelected(index);setEditingSlot({scope:'program',index:0,field:'condition'});}}><span className="unit-dot" style={{background:unit.color}}/><span>{unit.name}<small>{unit.role}</small></span><b>{unit.code}</b></button>)}</div>
        <div className="unit-meta"><div><span>{selectedUnit.code}</span><h3>{selectedUnit.name}</h3><small>{attackTypeLabels[selectedUnit.attackType]} / {rarityLabels[selectedUnit.rarity]} / 枠 {selectedUnit.programLimit}</small></div><div className="stats"><span>HP <b>{selectedUnit.maxHp}</b></span><span>ATK <b>{selectedUnit.attack}</b></span><span>RNG <b>{selectedUnit.range}</b></span><span>KB <b>{selectedUnit.knockbackPower}</b></span><span>WT <b>{selectedUnit.weight}</b></span><span>SPD <b>{selectedUnit.speed}</b></span></div><button className="sell-unit" onClick={sellSelected}>売却 +{Math.max(ECONOMY_CONFIG.minimumSellPrice,selectedUnit.price-ECONOMY_CONFIG.sellPricePenalty)}</button><button className="bench-unit" onClick={()=>moveTeamUnitToBench(selected)}>外す</button></div>
        <div className="mode-label"><span>NORMAL LOOP</span><b>クールダウン完了時に上から評価</b></div>
        <div className="program-list sentence-list">{currentProgram.map((block,index)=>{const instruction=instructionById.get(block.actionId)!;const conditionActive=editingSlot?.scope==='program'&&editingSlot.index===index&&editingSlot.field==='condition';const actionActive=editingSlot?.scope==='program'&&editingSlot.index===index&&editingSlot.field==='action';return <article className={`code-block sentence-block ${instruction.tone} ${block.fixedAction?'fixed-action':''}`} key={`${block.actionId}-${block.conditionId}-${index}`}><div className="line-no">{index+1}</div><div className="sentence-copy"><span>もし</span><button className={conditionActive?'word-slot active':'word-slot'} onClick={()=>setEditingSlot({scope:'program',index,field:'condition'})}>{conditionLabel(block.conditionId)}</button><span>なら</span><button className={actionActive?'word-slot active':'word-slot'} disabled={block.fixedAction} onClick={()=>setEditingSlot({scope:'program',index,field:'action'})}>{actionLabel(block.actionId)}</button></div><div className="line-actions"><button aria-label="上へ移動" disabled={index===0} onClick={()=>moveInstruction(index,-1)}><ArrowUp size={13}/></button><button aria-label="下へ移動" disabled={index===currentProgram.length-1} onClick={()=>moveInstruction(index,1)}><ArrowDown size={13}/></button><button aria-label="削除" disabled={block.fixedAction} onClick={()=>removeInstruction(index)}><X size={13}/></button></div></article>;})}<button className="add-block" onClick={()=>addInstruction(ownedActions[0])}>＋ 通常作戦を追加</button></div>
        <section className="reaction-loop">
          <div className="mode-label reaction-mode-label"><span>REACTION LOOP</span><b>常時監視 · {currentReaction?'1 / 1':'0 / 1'} · CD {BATTLE_CONFIG.reactionCooldownSeconds.toFixed(1)} SEC</b></div>
        ...
        <div className="choice-panel">
          ...
          <div className="choice-list">{editingSlot?.field==='condition'
            ? editingSlot.scope==='reaction'
              ? REACTION_TRIGGERS.map(trigger=><ConditionChoiceCard key={trigger.id} title={trigger.title} flavor={trigger.flavor} effect={trigger.effect} reaction active={currentReaction?.trigger===trigger.id} onSelect={()=>replaceFocusedSlot(trigger.id)}/>)
              : ownedConditions.map(condition=>{const detail=conditionById.get(condition)!;return <ConditionChoiceCard key={condition} title={detail.label} flavor={detail.flavor} effect={detail.effect} active={currentProgram[editingSlot.index]?.conditionId===condition} onSelect={()=>replaceFocusedSlot(condition)}/>;})
        ...
        <div className="inventory-grid">
          <div className="inventory"><small>控え</small><div>{bench.length===0?<span className="empty-inventory">なし</span>:bench.map(unit=><button key={unit.inventoryId} onClick={()=>equipBenchUnit(unit.inventoryId)}><span className="unit-mini" style={{background:unit.color}}/>{unit.name}</button>)}</div></div>
        </div>
      </section>
      <aside className="shop-panel"><div className="section-head"><div><span className="step-no">02</span><h2>ショップ</h2></div><button className="refresh" onClick={refresh}><RefreshCw size={15}/>更新 <b>{ECONOMY_CONFIG.refreshCost}</b></button></div><div className="shop-grid">{shop.map(item=>{const data:UnitDefinition|Instruction=item.kind==='unit'?unitById.get(item.id)!:instructionById.get(item.id)!;const isUnit=item.kind==='unit';const unit=isUnit?data as UnitDefinition:null;const label=unit?unit.name:(data as Instruction).title;const instruction=isUnit?null:data as Instruction;return <article className={`shop-item rarity-${data.rarity} ${instruction?'instruction-shop-item':''}`} key={item.key}><button className="lock" aria-label="ロック" onClick={()=>toggleLock(item.key)}>{item.locked?<Lock/>:<LockOpen/>}</button><small>{rarityLabels[data.rarity]} / {unit?attackTypeLabels[unit.attackType]:actionKindLabels[instruction!.action]}</small><strong>{label}</strong>{instruction?<><p className="shop-flavor">{instruction.flavor}</p><div className="shop-metrics">{instructionMetrics(instruction,selectedUnit).map(metric=><span key={metric.label}><small>{metric.label}</small><b>{metric.value}</b></span>)}</div></>:unit?<><p>{`ATK ${unit.attack} / SPD ${unit.speed} / RNG ${unit.range} / 枠 ${unit.programLimit}`}</p><ShopUnitTrait unit={unit}/></>:null}<button className="buy" onClick={()=>buy(item)}><ShoppingCart size={15}/><span>購入</span><b><Coins size={13}/>{data.price}</b></button></article>;})}</div><button className="ready" onClick={startBattle}><Swords/>戦闘開始</button></aside>
    </div>:<div className="battle-layout">
      <section className="arena"><div className="battle-hud"><div className="hud-team ally"><small>YOUR SQUAD</small><b>{teamHp}</b><span>HP</span></div><div className="timer"><small>{elapsed>=BATTLE_CONFIG.overheatStartSeconds-BATTLE_CONFIG.overheatWarningSeconds?'OVERHEAT IN':'BATTLE TIME'}</small><b>{Math.floor(elapsed/60)}:{String(Math.floor(elapsed%60)).padStart(2,'0')}</b></div><div className="hud-team enemy"><small>ENEMY</small><b>{enemyHp}</b><span>HP</span></div></div><BattleScene fighters={fighters} flash={flash} running={phase==='battle'&&!paused}/><div className="unit-bars">{fighters.map(fighter=><div className={fighter.team} key={fighter.instanceId}><span>{fighter.name}</span><div><i style={{width:`${Math.max(0,fighter.hp/fighter.maxHp*100)}%`}}/></div><b>{Math.ceil(fighter.hp)}</b></div>)}</div><div className="battle-controls"><button onClick={()=>setPaused(current=>!current)}>{paused?<Play/>:<Pause/>}</button><button className={speed===1?'active':''} onClick={()=>setSpeed(1)}>x1</button><button className={speed===2?'active':''} onClick={()=>setSpeed(2)}>x2</button><button onClick={reset}><RotateCcw/></button></div></section>
      <aside className="status-panel"><div className="section-head"><div><span className="live-dot"/><h2>ユニット状態</h2></div><button className="open-log" onClick={()=>setLogsOpen(true)}>ログ <b>{logs.length}</b></button></div><div className="status-roster">{fighterGroups.map(group=><section className="status-group" key={group.key}><h3>{group.label}</h3>{group.units.map(fighter=>{const hpRatio=Math.max(0,fighter.hp/fighter.maxHp*100);const cooldownRatio=cooldownProgress(fighter);const active=flash?.id===fighter.instanceId&&flash.actionLabel;return <article className={`unit-status-card unit-${fighter.id} ${fighter.team} ${fighter.hp<=0?'down':''} ${fighter.berserk?'berserk':''} ${active?'acting':''}`} key={fighter.instanceId}><div className="status-avatar" style={{['--unit-color' as string]:fighter.color}}><i/></div><div className="status-id"><small>{fighter.code}</small><strong>{fighter.name}</strong><span>{attackTypeLabels[fighter.attackType]}</span></div><div className="unit-status-hp"><div><i style={{width:`${hpRatio}%`}}/></div><b>{Math.ceil(Math.max(0,fighter.hp))}/{fighter.maxHp}</b></div><div className="status-cooldown"><div><i style={{width:`${cooldownRatio*100}%`}}/></div><b>{cooldownRatio>=1?'READY':'WAIT'}</b></div><div className="status-stats"><span>A <b>{fighter.attack}</b></span><span>R <b>{fighter.range}</b></span><span>K <b>{fighter.knockbackPower}</b></span><span>W <b>{fighter.weight}</b></span><span>D <b>{fighter.defense}</b></span><span>S <b>{fighter.speed}</b></span></div><div className="status-tags">{statusTags(fighter).map(tag=><span className={tag==='正常'?'normal':tag==='暴走'?'berserk':''} key={tag}>{tag}</span>)}</div>{active&&<div className={`card-action-bubble ${flash?.kind==='miss'?'miss':''}`}>{flash.actionLabel}</div>}</article>;})}</section>)}</div></aside>
      {logsOpen&&<div className="log-dialog-overlay" role="dialog" aria-modal="true" aria-label="戦闘ログ"><div className="log-dialog"><div className="log-dialog-head"><div><span className="live-dot"/><h2>戦闘ログ</h2><small>{logs.length} EVENTS</small></div><button aria-label="ログを閉じる" onClick={()=>setLogsOpen(false)}><X size={18}/></button></div><div className="logs">{logs.length===0?<div className="empty-log"><Sparkles/><span>プログラムを初期化中</span></div>:logs.map(log=><div className={`log ${log.type}`} key={log.id}><time>{log.time}</time><div><b>{log.actor}</b><span>{log.text}</span></div></div>)}</div></div></div>}
      {phase==='result'&&<div className="result-overlay"><div className="result-dialog"><small>BATTLE COMPLETE</small><h1>{winner}</h1><p>{winner==='勝利'?'ロジックが敵チームを停止させました':'プログラムを調整して再実行できます'}</p><div><span>実行イベント <b>{logs.length}</b></span><span>戦闘時間 <b>{elapsed.toFixed(1)}s</b></span></div><button onClick={()=>{setRound(current=>current+1);setCoins(current=>current+ECONOMY_CONFIG.roundReward);reset();}}>次のラウンドへ</button></div></div>}
    </div>}
